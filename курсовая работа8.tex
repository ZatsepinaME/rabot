...
\      $где $x$ \in {\,} $[{x_i},{x_{i+1}}].$\\[4pt]
    ...
    \end{equation}
     ...
     {\normalsize ${F(x)}$= \left\{ 
      	\begin{aligned}
      	$F_1(x)&=0.634x^3-5.154x^2+15.708x-12.708,$\,\, $где$ \,\,\,{x}\,{\in}\,\,\, {$[0,\,0.5]$}};\\
      $F_2(x)&=1.849x^3-3.258x^2+0.504x+2.196,$\,\, $где$ \, {x}\,{\in}\, {$[0.5,\,1.4]$}};\\
    $F_3(x)&=1.768x^3+2.849x^2-9.357x+13.057,$\,\, $где$ \,\,\,{x}\,{\in}\,\,\, {$[1.4,\,2.25]$}};\\
$F_4(x)&=2.569x^3-0.021x^2-0.787x+4.12$,\,\, $где$\,{x}\,{\in}\, {$[2.25,\,3.5]$}}.\\
      	\end{aligned} 	\right.\]
   }\\[20pt]
   ${F(1.2)}=19.579$
    \\[100pt]
...
ylabel = {$y$},
grid=major]]
\addplot[domain=0:0.5,smooth] {0.634*x^3+(-5.154)*x^2+(15.708)*x-12.708};
\addplot[domain=0.5:1.4,smooth] {1.849*x^3-3.2*x^2+(0.504)*x-5.29};
\addplot[domain=1.4:2.25,smooth] {1.768*x^3+2.849*x^2-9.357*x-3.757};
\addplot[domain=2.25:3.5,smooth] {2.569*x^3-0.021*x^2+(-0.787)*x-17.52};
%\newcommand{\xx}{0.5}
\draw (152,6.0) circle (1.8pt);
\draw (236.5,6.0) circle (1.8pt);
\draw (323,22.0) circle (1.8pt);
\end{axis}
\end{scope}
\end{tikzpicture}\\
      	  \newpage
